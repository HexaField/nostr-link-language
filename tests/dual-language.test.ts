/**
 * Tests for dual-language deduplication and origin tracking.
 */
...
describe("shouldFederate", () => {
    ...
    it("returns false for nostr-origin links (prevents echo loop)", () => {
        const getOrigin = (key: string): string | null => {
            if (key === "link-origin/hash789") return "nostr";
            return null;
        };
...
describe("isPredicateExcluded", () => {
    it("returns true for excluded predicate", () => {
        assert.equal(
            isPredicateExcluded("system://internal", ["system://internal"]),
            true,
        );
    });
    ...
    it("returns false for non-excluded predicate", () => {
        assert.equal(
            isPredicateExcluded("flux://has_message", ["system://internal"]),
            false,
        );
    });

    it("returns false for empty exclusion list", () => {
        assert.equal(isPredicateExcluded("any://pred", []), false);
    });
});
